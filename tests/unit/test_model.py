...
class TestPatternSpecValidation:
    def test_minimal_valid_spec(self, sample_pattern_spec):
        spec = PatternSpec.model_validate(sample_pattern_spec)
        assert spec.cpm == 20
        assert len(spec.layers) == 2

    def test_duplicate_orbits_raises(self, sample_pattern_spec):
    ...
    def test_zero_layers_raises(self, sample_pattern_spec):
        sample_pattern_spec["layers"] = []
        with pytest.raises(ValidationError) as exc:
            PatternSpec.model_validate(sample_pattern_spec)
        assert "too_short" in str(exc.value)

    def test_default_values_applied(self, sample_pattern_spec):
        spec = PatternSpec.model_validate(sample_pattern_spec)
        for layer in spec.layers:
            assert layer.slow_factor >= 4
            assert layer.room >= 0.7

    def test_oscillator_type_enum(self, sample_pattern_spec):
        spec = PatternSpec.model_validate(sample_pattern_spec)
